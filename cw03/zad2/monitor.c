...
int run_monitoring(struct monitored_file **monitored_files, int monitored_numb, double time_of_running, enum copy_mode mode){
    for(int i=0; i<monitored_numb; i++){
        pid_t child_pid;
        child_pid = fork();
        if(child_pid == 0){
...
int main(int argc, char **argv){
    ...
    return run_monitoring(monitored_files,monitored_numb,time_of_running,mode);
}